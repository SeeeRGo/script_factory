...
- [правила версионирования каждого коммита](docs/versioning.md);
...
Первый кейс одновременно показывает P10 и P50 в очереди, затем запускает P10 первым,
проводит его через две попытки и завершает ошибкой. Во втором P10 поступает во время
второй попытки активного P50 и ждёт освобождения слота: очередь невытесняющая. Runner
выводит `attempt/max_attempts` для каждого перехода. Полный маршрут показа описан в
